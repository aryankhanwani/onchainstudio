...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Generate random grayscale colors using HSV to RGB
      // Hue doesn't matter for grayscale, saturation = 0 (no color), value = brightness (0 = black, 1 = white)
      const randomHue = Math.random(); // Can be any value, doesn't affect grayscale
      const saturation = 0; // 0 = grayscale (no color)
      // Generate value with better distribution: mix of light, medium, and dark grays
      const rand = Math.random();
      let value;
      if (rand < 0.33) {
        // Light grays to white (0.7 to 1.0)
        value = 0.7 + Math.random() * 0.3;
      } else if (rand < 0.66) {
        // Medium grays (0.3 to 0.7)
        value = 0.3 + Math.random() * 0.4;
      } else {
        // Dark grays to black (0.0 to 0.3)
        value = Math.random() * 0.3;
      }
      const c = HSVtoRGB(randomHue, saturation, value);
      // Reduce opacity by multiplying RGB values (0.5 = 50% opacity)
      const opacity = 0.1;
      c.r *= opacity;
      c.g *= opacity;
      c.b *= opacity;
